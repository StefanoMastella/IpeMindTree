...
import { users, ideas, comments, resources } from "@shared/schema";
...
export class DatabaseStorage implements IStorage {
  ...
  async updateIdeaConnections(id: number, connectionIds: number[]): Promise<Idea> {
    ...
    const validConnectionIds = await Promise.all(
      connectionIds.filter(async connId => {
        if (connId === id) return false;
        const connectedIdea = await this.getIdea(connId);
    ...
    for (const connId of validConnectionIds) {
      const connectedIdea = await this.getIdea(connId);
      if (connectedIdea && !connectedIdea.connections.includes(id)) {
        const newConnections = [...connectedIdea.connections, id];
        await db
          .update(ideas)
