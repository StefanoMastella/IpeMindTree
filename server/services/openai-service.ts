...
import { getMainPrompt } from "../../client/src/lib/llm-context";
...
export class OpenAIService {
  ...
  /**
   * Main method to query with a user question
  ...
  /**
   * Query with a user question and conversation history
...
`;
      ...
      const userPrompt = `${chatHistory ? `## Previous conversation history:
${chatHistory}
...
User question: ${userQuestion}`;
      ...
      console.log("Calling OpenAI API with" + (selectedSubprompt ? " selected subprompt..." : "out subprompt..."));
      ...
      return completion.choices[0].message.content || "I couldn't generate a proper response. Please try again with a different question.";
    } catch (error) {
...
`;
      ...
      return completion.choices[0].message.content || "Could not generate a summary for this idea.";
    } catch (error) {
